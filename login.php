...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["password"]))) {
    ...
    // Verificar si el dispositivo está vacío
  if (empty(trim($_POST["device"]))) {
    $login_err = "Por favor, ingrese su dispositivo.";
  } else {
        ...
        if (mysqli_stmt_num_rows($stmt) == 1) {
          ...
          if (mysqli_stmt_fetch($stmt)) {
            ...
            if (password_verify($password, $hashed_password)) {
              ...
              $param_device = $device; // Usar la variable $device que obtuviste del formulario
...
<head>
    ...
    <style>
        ...
        .wrapper{ width: 360px; padding: 20px; margin: 0 auto; } /* Añadido margin: 0 auto; para centrar el formulario */
    </style>
</head>
<body>
    <div class="wrapper">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="form-group">
                ...
                <input type="text" name="username" class="form-control <?php echo (!empty($login_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $username; ?>">
                <span class="invalid-feedback"><?php echo $login_err; ?></span>
            </div>
            <div class="form-group">
                <label>Contraseña</label>
                <input type="password" name="password" class="form-control <?php echo (!empty($login_err)) ? 'is-invalid' : ''; ?>">
                <span class="invalid-feedback"><?php echo $login_err; ?></span>
            </div>
            <div class="form-group">
                <label>Dispositivo</label>
                <input type="text" name="device" class="form-control <?php echo (!empty($login_err)) ? 'is-invalid' : ''; ?>" value="<?php echo $device; ?>">
                <span class="invalid-feedback"><?php echo $login_err; ?></span>
            </div>
            <div class="form-group">
